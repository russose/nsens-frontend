...
import { buildListStringSeparated } from "./fetchWikiBase";
...
export async function buildVitalStaticKnowbooksAllLanguages(
  ...
  const vitalKnowbooksAllLanguages_FirstLetterCap =
    vitalKnowbooksAllLanguages.map((knowbook) => {
      ...
      knowbook_.name = capitalizeFirstLetter(knowbook_.name);
      return knowbook_;
    });
...
async function vitalStaticKnowbooksAllLanguages(
  vitalKnowbooksEnglish: IKnowbook[],
  ROOT_URL_WIKIDATA_ACTION: string,
  langs: Tlanguage[]
): Promise<IKnowbook[]> {
  ...
  const vitalKnowbooksAllLanguages: IKnowbook[] = [...vitalKnowbooksEnglish];

  for (const knowbookEnglish of vitalKnowbooksEnglish) {
    const cathegory_to_find_in_other_languages: string = knowbookEnglish.name;
...
async function buildVitalStaticKnowbooksEnglish(
  ...
  const vitalKnowbooks: IKnowbook[] = [];
