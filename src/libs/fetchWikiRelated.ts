...
// const relation_name_wikidata = configGeneral.relation_name_wikidata;
...
export async function fetchWikipediaRelatedWithoutImage(
  itemId: AtomID,
  title: string,
  ...
  relation_name_wikipedia: string
): Promise<IRelatedAtomFull[]> {
  if (itemId === undefined || title === undefined) {
    return;
  }

  const items_wikipedia: IAtom[] = await ItemsRelatedFromWikipediaWithoutImage(
    title,
    // itemId,
    amount_wikipedia,
    ROOT_URL_REST_API,
  ...
  const relatedItems_wikipedia: IRelatedAtomFull[] = items_wikipedia.map(
    (item: IAtom) => {
      return { relation: relation_name_wikipedia, item: item };
    }
  );

  return relatedItems_wikipedia;
}

export async function fetchWikidataRelatedWithoutImage(
  itemId: AtomID,
  title: string,
  // amount_wikipedia: number,
  amount_max_by_node_wikidata: number,
  ...
  lang: Tlanguage,
  exclusion_patterns: string[],
  relation_name_wikidata: string
): Promise<IRelatedAtomFull[]> {
  if (itemId === undefined || title === undefined) {
    return;
  }

  let relatedItems_wikidata = await ItemsFromWikidataWithoutImage(
    itemId,
  ...
  relatedItems_wikidata = relatedItems_wikidata.map((related) => {
    return {
      relation:
        relation_name_wikidata +
        relation_multilevel_separator +
        related.relation,
      item: related.item,
    };
  });
  ...
  // relatedItems_wikidata.forEach((related) => {
  //   relatedItems_no_doubles.set(related.item.id, related);
  // });
  // const relatedItems_no_doubles_array = Array.from(
  //   relatedItems_no_doubles.values()
  // );

  return relatedItems_wikidata;
}

// export async function fetchWikiRelatedWithoutImage(
//   itemId: AtomID,
//   title: string,
//   amount_wikipedia: number,
//   amount_max_by_node_wikidata: number,
